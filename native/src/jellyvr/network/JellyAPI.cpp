...
Ref<Json> JellyAPI::fetch_recently_added_items_from_collection(Node *node, String collection_id, int num_of_items, Ref<AppConfig> app_config, Ref<NetworkConfig> network_config) {
    if(num_of_items < 0 || num_of_items > 10) {
        ...
        String http_url = network_config->get_server_url();
        String user_id = app_config->get_user_id();
        String endpoint = "/Items?ParentId=" + collection_id + "&Fields=BasicSyncInfo,Overview,Genres,UserData,RemoteTrailers&ExcludeItemTypes=Folder&SortBy=DateCreated&SortOrder=Descending&isPlayed=false&Limit=" + UtilityFunctions::str(num_of_items)  +"&Recursive=true&userId=" + user_id;
        String full_url = http_url + endpoint;
